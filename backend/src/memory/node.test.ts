...
describe('HeapInBytes', () => {
  ...
  test('should correctly set heap bytes', () => {
    const tag = 1;
    const child = new Array(HEAP_NODE_BYTE_SIZE.child).map((_, idx) => idx + 50); // Assuming this is not the expected size
    const data = new Array(HEAP_NODE_BYTE_SIZE.data).map((_, idx) => idx + 80);

    expect(HeapInBytes.fromBytes([tag, ...child, ...data])._bytes).toEqual([tag, ...child, ...data]);
  });

  test('should throw error for invalid byte size', () => {
    const bytes = [1, 2, 3]; // Assuming this is not the expected size

    expect(() => HeapInBytes.fromBytes([...bytes])).toThrow(`Invalid byte size: ${bytes.length}`);
  });
  ...
  test('should correctly convert string to bytes', () => {
    ...
    for (const data of simpleData) {
      ...
      const h = HeapInBytes.fromData({ type: Type.String, gcFlag: GcFlag.Unmarked, data, child: nextAddr });
      const bytes = h.toBytes();
      const converted = HeapInBytes.fromBytes(bytes).toData() as any;
      expect(stringNormalizer(converted.data)).toStrictEqual(stringNormalizer(data));
  ...
  test('should correctly convert symbol to bytes', () => {
    ...
    for (const data of simpleData) {
      ...
      const h = HeapInBytes.fromData({ type: Type.Symbol, gcFlag: GcFlag.Unmarked, data, child: nextAddr });
      const bytes = h.toBytes();
      const converted = HeapInBytes.fromBytes(bytes).toData() as any;
      expect(stringNormalizer(converted.data)).toStrictEqual(stringNormalizer(data));
      expect(converted.child).toStrictEqual(nextAddr);
  ...
  test('should correctly convert lambda to bytes', () => {
    ...
    const h = HeapInBytes.fromData({ type: Type.Lambda, gcFlag: GcFlag.Unmarked, data: pcAddr, child: nextAddr });
    const bytes = h.toBytes();
    const converted = HeapInBytes.fromBytes(bytes).toData() as any;
  ...
  test('should correctly convert frameAddr to bytes', () => {
    ...
    const h = HeapInBytes.fromData({ type: Type.FrameAddr, gcFlag: GcFlag.Unmarked, data: dataAddr, child: nextAddr });
    const bytes = h.toBytes();
    const converted = HeapInBytes.fromBytes(bytes).toData() as any;
  ...
  test('should correctly convert value to bytes', () => {
    const nextAddr = HeapAddr.fromNum(9);
    const dataAddr = HeapAddr.fromNum(10);
    const h = HeapInBytes.fromData({ type: Type.Value, gcFlag: GcFlag.Unmarked, data: dataAddr, child: nextAddr });
    const bytes = h.toBytes();
    const converted = HeapInBytes.fromBytes(bytes).toData() as any;
    expect(converted.data).toStrictEqual(dataAddr);
    expect(converted.child).toStrictEqual(nextAddr);
  ...
  test('should correctly convert heapAddr to bytes', () => {
    const nextAddr = HeapAddr.fromNum(9);
    const h = HeapInBytes.fromData({ type: Type.HeapAddr, gcFlag: GcFlag.Unmarked, child: nextAddr });
    const bytes = h.toBytes();
    const converted = HeapInBytes.fromBytes(bytes).toData() as any;
    expect(converted.child).toStrictEqual(nextAddr);
