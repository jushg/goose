...
export type HeapTypesWithChildren = Type.String | Type.Lambda | Type.FrameAddr | Type.Symbol | Type.Value | Type.HeapAddr;
...
export class HeapInBytes {
   ...
   private constructor(tag: number, child: number[], data: number[]) {
    ...
    static fromBytes(bytes: number[]): HeapInBytes {
      if (bytes.length !== HEAP_NODE_BYTE_SIZE.child + HEAP_NODE_BYTE_SIZE.data + HEAP_NODE_BYTE_SIZE.tag) {
        throw new Error(`Invalid byte size: ${bytes.length}`);
      }
      
      const tag = bytes[HEAP_NODE_BYTE_INDICES.tag]
      const child = bytes.slice(HEAP_NODE_BYTE_INDICES.child, HEAP_NODE_BYTE_INDICES.child + HEAP_NODE_BYTE_SIZE.child)
      const data = bytes.slice(HEAP_NODE_BYTE_INDICES.data, HEAP_NODE_BYTE_INDICES.data + HEAP_NODE_BYTE_SIZE.data)
    ...
    toData(): AnyHeapValue {
      ...
      const child = this._bytes.slice(HEAP_NODE_BYTE_INDICES.child, HEAP_NODE_BYTE_INDICES.child + HEAP_NODE_BYTE_SIZE.child)
      const data = this._bytes.slice(HEAP_NODE_BYTE_INDICES.data, HEAP_NODE_BYTE_INDICES.data + HEAP_NODE_BYTE_SIZE.data)
    ...
    toString(): string {
      const { type, gcFlag } = HeapInBytes.fromTagByte(this._bytes[HEAP_NODE_BYTE_INDICES.tag])
      const child = this._bytes.slice(HEAP_NODE_BYTE_INDICES.child, HEAP_NODE_BYTE_INDICES.child + HEAP_NODE_BYTE_SIZE.child)
      const childAsHex = child.map((x) => x.toString(16).padStart(2, "0")).join("")
      const data = this._bytes.slice(HEAP_NODE_BYTE_INDICES.data, HEAP_NODE_BYTE_INDICES.data + HEAP_NODE_BYTE_SIZE.data)
